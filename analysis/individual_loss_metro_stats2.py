...
for k, v in shrinking_metros.iteritems():
	...
	END loss_flag,
	CASE 
		WHEN B.black00 > B.white00 AND (B.black00 + B.white00) * 1.0 / B.pop00 >= 0.8 THEN 'black_dom'
		WHEN B.black00 < B.white00 AND (B.black00 + B.white00) * 1.0 / B.pop00 >= 0.8 THEN 'white_dom'
		ELSE 'other'
	END race_cat
	...
	temp = temp.groupby(['name10','race_cat', 'loss_flag']).sum()
	lst = ['00', '10']
	for l in lst:
		cols = ['white{}'.format(l), 'black{}'.format(l), 'asian{}'.format(l), 'hisp{}'.format(l), 'other{}'.format(l)]
		for c in cols:
...
outf = "/home/eric/Documents/franklin/fowler/individual_loss_metro_stats_race.xlsx"
