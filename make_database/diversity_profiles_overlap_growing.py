...
qry = '''
SELECT DISTINCT A.*
FROM subset_places_80_90 AS A JOIN all_cities_50k AS B
ON A.cbsa_geoid10 = B.cbsa_geoid10
LEFT JOIN shrinking_cities AS C ON B.NHGISCODE = C.NHGISCODE
WHERE C.NHGISCODE IS NULL;
'''
df90 = pd.read_sql(qry, con)

qry = '''
SELECT DISTINCT A.*
FROM subset_places_00_10 AS A JOIN all_cities_50k AS B
ON A.cbsa_geoid10 = B.cbsa_geoid10
LEFT JOIN shrinking_cities AS C ON B.NHGISCODE = C.NHGISCODE
WHERE C.NHGISCODE IS NULL;
'''
...
fig, axes = plt.subplots(nrows=12, ncols=5, sharex=True, sharey=True, figsize=(16,30), squeeze=False)
axli = axes.flatten()
...
for k, v in cbsa_dict.iteritems():
	if count>=57:
		...
		ax=sns.kdeplot(data=temp90['Ep'], label='all 1990', ax=axli[count], legend=False, color='#377eb8')
		sns.kdeplot(data=temp10['Ep'], label='all 2010', ax=axli[count], legend=False, color='#e41a1c')

		# get E for each shrinking city in CBSA
		qry = '''
		SELECT A.nhgisplace90 as nhgisplace, Ep
		FROM subset_places_80_90 AS A JOIN all_cities_50k AS B
		...
		for i, x in temp90_2.iterrows():
			if count2==0:
				ax.axvline(x['Ep'], color='#377eb8', alpha=0.5, label='growing 1990')
			else:
				ax.axvline(x['Ep'], color='#377eb8', alpha=0.5)
			count2+=1


		# get E for each shrinking city in CBSA
		qry = '''
		SELECT A.nhgisplace10 as nhgisplace, Ep
		FROM subset_places_00_10 AS A JOIN all_cities_50k AS B
		...
		for i, x in temp10_2.iterrows():
			if count2==0:
				ax.axvline(x['Ep'], color='#e41a1c', alpha=0.5, label='growing 2010')
			else:
				ax.axvline(x['Ep'], color='#e41a1c', alpha=0.5)
			count2+=1
...
outFile = "/home/eric/Documents/franklin/fowler/figures/diversity_profiles_shrinking_cities_overlap_growth.png"
